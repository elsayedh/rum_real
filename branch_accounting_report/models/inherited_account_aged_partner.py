...
class AccountAgedPartner(models.AbstractModel):
    ...
    def _get_columns_name(self, options):
        columns = [
            ...
            {'name': _("Branch"), 'class': '', 'style': 'text-align:center; white-space:nowrap;'},
            ...
            if 'partner_%s' % (values['partner_id'],) in options.get('unfolded_lines'):
                for line in amls[values['partner_id']]:
                    ...
                    vals = {
                        ...
                        'columns': [{'name': v} for v in [format_date(self.env, aml.date_maturity or aml.date), aml.journal_id.code, aml.account_id.display_name, aml.branch_id.name or '', format_date(self.env, aml.expected_pay_date)]] +
...
class ReportAgedPartnerBalance(models.AbstractModel):
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move, period_length):
        ...
        for i in range(5):
            if branch_ids:
                args_list = (tuple(move_state), tuple(account_type), tuple(partner_ids), tuple(branch_ids))
            else:
        ...
        if branch_ids:
            query = '''SELECT l.id
                ...
                AND l.company_id IN %s
                ORDER BY COALESCE(l.date_maturity, l.date)'''
            cr.execute(query, (tuple(move_state), tuple(account_type), date_from, tuple(partner_ids), tuple(branch_ids), date_from, tuple(company_ids)))
        else:
            query = '''SELECT l.id
                FROM account_move_line AS l, account_account, account_move am
                WHERE (l.account_id = account_account.id) AND (l.move_id = am.id)
