...
# https://docs.moyasar.com/developers/development-resources/currency-codes
CURRENCY_CODE_MAPS = {
    "BHD": 3,
    "CVE": 0,
    "DJF": 0,
...
class AcquirerMoyasar(models.Model):
    _inherit = 'payment.acquirer'

    provider = fields.Selection(selection_add=[('moyasar', 'Moyasar')])
    moyasar_merchant_account = fields.Char('Merchant Account', required_if_provider='moyasar', groups='base.group_user')
    Moyasar_publishable_api_key = fields.Char('publishable api key ', required_if_provider='moyasar', groups='base.group_user',defualt="in")
    Moyasar_scret_key = fields.Char('Secret Key', required_if_provider='moyasar', groups='base.group_user')
    moyasar_company_url = fields.Char('URL', required_if_provider='moyasar', groups='base.group_user')


    @api.model
    def _moyasar_convert_amount(self, amount, currency):
        """
    ...
    @api.model
    def _get_moyasar_urls(self, environment):
        """ Moyasar URLs: yhpp: hosted payment page: pay.shtml for single, select.shtml for multiple """
        return {
            'moyasar_form_url': '/payment/moyasar',
        }

    def _moyasar_generate_merchant_sig_sha256(self, inout, values):
        ...
        assert self.provider == 'moyasar'

        if inout == 'in':
            # All the fields sent to Moyasar must be included in the signature. ALL the fucking
            # fields, despite what is claimed in the documentation. For example, in
            # https://docs.moyasar.com/developers/hpp-manual, it is stated: "The resURL parameter does
        ...
        hmac_key = binascii.a2b_hex(self.Moyasar_scret_key.encode('ascii'))
        ...
        print(" Hi ya basha " , raw_values_ordered)
        return signParams(raw_values_ordered)

    def _moyasar_generate_merchant_sig(self, inout, values):
        ...
        key = self.Moyasar_scret_key.encode('ascii')
        print(" Hi ya basha2 sign", sign)
        print(" Hi ya basha2 key", key)
        print(" Hi ya basha2 b64encode", base64.b64encode(hmac.new(key, sign, hashlib.sha1).digest()))
    ...
    def moyasar_form_generate_values(self, values):
        ...
        paymentAmount = self._moyasar_convert_amount(values['amount'], values['currency'])
        if self.provider == 'moyasar' and len(self.Moyasar_scret_key) == 64:
            ...
            values.update({
                ...
                'skinCode': self.Moyasar_publishable_api_key,
                'merchantAccount': self.moyasar_merchant_account,
            ...
            values['merchantSig'] = self._moyasar_generate_merchant_sig_sha256('in', values)
            ...
            values['merchantSig'] = self._moyasar_generate_merchant_sig('in', values)

        return values

    def moyasar_get_form_action_url(self):
        self.ensure_one()
        environment = 'prod' if self.state == 'enabled' else 'test'
        return self._get_moyasar_urls(environment)['moyasar_form_url']


class TxMoyasar(models.Model):
    ...
    def _moyasar_form_get_tx_from_data(self, data):
        ...
        if not reference or not pspReference:
            error_msg = _('Moyasar: received data with missing reference (%s) or missing pspReference (%s)') % (reference, pspReference)
        ...
        if not tx or len(tx) > 1:
            error_msg = _('Moyasar: received data for reference %s') % (reference)
            if not tx:
    ...
    def _moyasar_form_get_invalid_parameters(self, data):
        print("invalid parameter ya shaliby ",data)
        invalid_parameters = []
        print(self.acquirer_reference)
        ...
        # # seller
        # if data.get('skinCode') != self.acquirer_id.Moyasar_publishable_api_key:
        #     invalid_parameters.append(('skinCode', data.get('skinCode'), self.acquirer_id.Moyasar_publishable_api_key))
        # # result
    ...
    def _moyasar_form_validate(self, data):
        print("Form validate ya shaliby ", data)
        ...
        if status == 'paid':
            ...
            error = _('Moyasar: feedback error')
            _logger.info(error)
